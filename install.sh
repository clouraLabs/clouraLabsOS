...
# Creates a complete flake-based NixOS configuration
...
echo "  1) GNOME (Modern, user-friendly)"
echo "  2) KDE Plasma (Feature-rich, customizable)"
echo "  3) XFCE (Lightweight, traditional)"
echo "  4) Cinnamon (Elegant, familiar)"
echo "  5) MATE (Classic GNOME 2 fork)"
echo "  6) i3 (Tiling window manager)"
echo "  7) Hyprland (Wayland tiling compositor)"
echo "  8) None (Minimal/Server)"
echo ""

while true; do
    DE_CHOICE=$(prompt_input "Choose desktop [1-8]" "1")
    case $DE_CHOICE in
...
if prompt_yes_no "Install development tools? (git, vim, vscodium, etc)"; then
...
if prompt_yes_no "Install virtualization? (QEMU/KVM)"; then
    INSTALL_VIRTUALIZATION=true
fi

if prompt_yes_no "Install AI tools? (Claude, Codex, Grok CLI)"; then
    INSTALL_AI_TOOLS=true
fi

if prompt_yes_no "Install gaming tools? (Steam, Proton, Lutris)"; then
    INSTALL_GAMING=true
fi

if prompt_yes_no "Install multimedia production? (OBS, Kdenlive, GIMP, Blender)"; then
    INSTALL_MULTIMEDIA=true
fi

if prompt_yes_no "Install cloud provider CLIs? (AWS, Azure, GCP)"; then
    INSTALL_CLOUD_TOOLS=true
fi

# Security hardening
print_header "Security Hardening"
if prompt_yes_no "Enable security hardening? (Firewall, fail2ban, AppArmor)"; then
    SECURITY_HARDENING=true
fi

# Additional users
print_header "Additional Users"
if prompt_yes_no "Create additional user accounts?"; then
    while true; do
        ADD_USER=$(prompt_input "Enter username (or press Enter to finish)" "")
        if [ -z "$ADD_USER" ]; then
            break
        fi
...
if prompt_yes_no "Install Home Manager for user configuration?"; then
...
echo -e "${CYAN}│${NC} Locale:           ${GREEN}$LOCALE${NC}"
echo -e "${CYAN}│${NC} Desktop:          ${GREEN}$DESKTOP${NC}"
echo -e "${CYAN}│${NC} Dev Tools:        $([ "$INSTALL_DEV_TOOLS" = true ] && echo -e "${GREEN}Yes${NC}" || echo -e "${RED}No${NC}")"
echo -e "${CYAN}│${NC} Docker:           $([ "$INSTALL_DOCKER" = true ] && echo -e "${GREEN}Yes${NC}" || echo -e "${RED}No${NC}")"
echo -e "${CYAN}│${NC} Virtualization:   $([ "$INSTALL_VIRTUALIZATION" = true ] && echo -e "${GREEN}Yes${NC}" || echo -e "${RED}No${NC}")"
echo -e "${CYAN}│${NC} AI Tools:         $([ "$INSTALL_AI_TOOLS" = true ] && echo -e "${GREEN}Yes${NC}" || echo -e "${RED}No${NC}")"
echo -e "${CYAN}│${NC} Gaming:           $([ "$INSTALL_GAMING" = true ] && echo -e "${GREEN}Yes${NC}" || echo -e "${RED}No${NC}")"
echo -e "${CYAN}│${NC} Multimedia:       $([ "$INSTALL_MULTIMEDIA" = true ] && echo -e "${GREEN}Yes${NC}" || echo -e "${RED}No${NC}")"
echo -e "${CYAN}│${NC} Cloud Tools:      $([ "$INSTALL_CLOUD_TOOLS" = true ] && echo -e "${GREEN}Yes${NC}" || echo -e "${RED}No${NC}")"
echo -e "${CYAN}│${NC} Security:         $([ "$SECURITY_HARDENING" = true ] && echo -e "${GREEN}Hardened${NC}" || echo -e "${YELLOW}Standard${NC}")"
echo -e "${CYAN}│${NC} Additional Users: ${GREEN}${#ADDITIONAL_USERS[@]}${NC}"
...
if prompt_yes_no "Generate hardware configuration? (requires sudo)" "y"; then
...
if [ "$GENERATE_HARDWARE" = false ]; then
    print_warning "Creating placeholder hardware configuration"
...
# You can generate it with: sudo nixos-generate-config --show-hardware-config
...
EOF
    print_success "Created placeholder hardware-configuration.nix"
    echo ""
...
{
  # Multimedia production packages
  environment.systemPackages = with pkgs; [
    # Video editing
    kdenlive
    shotcut
...
$([ "$INSTALL_GAMING" = true ] && echo "│   ├── gaming.nix                 # Gaming (Steam, Lutris)")
$([ "$INSTALL_MULTIMEDIA" = true ] && echo "│   ├── multimedia.nix             # Multimedia production")
$([ "$INSTALL_CLOUD_TOOLS" = true ] && echo "│   ├── cloud-tools.nix            # Cloud provider CLIs")
$([ "$SECURITY_HARDENING" = true ] && echo "│   └── security.nix                # Security hardening")
└── users/
    └── $USERNAME/
...
if [ ${#ADDITIONAL_USERS[@]} -gt 0 ]; then
    print_info "📝 Additional Users Created:"
    for user in "${ADDITIONAL_USERS[@]}"; do
        echo "   - $user"
    done
    echo ""
...
if [ "$SECURITY_HARDENING" = true ]; then
    print_warning "🔒 Security Hardening Enabled"
...
echo "2. Review the generated files (especially hardware-configuration.nix)"
echo ""
